...
exports.payWithCredit = async function(client, contactid, orderid, paymentmethodid, paymentAmount) {
    const { vouchers } = await contacts.getCreditInfo(client, contactid)
...
exports.payWithVoucher = async function(client, voucher, orderid, paymentmethodid, amount) {
  const vouchercode = voucher.code
  const vouchercodeid = voucher.id
  console.log(`[${client.shortname}] Going to pay ${amount} with voucher ${vouchercode} in order ${orderid}`)
  const payload = {
      amount,
      paymentmethodid,
      vouchercode,
  ...
  return api.post(client, "payments", orderid, payload)
}

async function getMollieRefundablePayments(client, orderid) {
    const sql = `
      select 
        p.id, 
        p.paidts, 
      ...
      left join (
          ...
          where m2.paymentmethodtypeid in (1006, 1014)
          and p2.amount < 0   
      ) refunds
          on refunds.orderid = p.orderid
          and refunds.mollieoriginalid = properties::json->>'id'
      where p.orderid = ${orderid}
      and paymentmethodtypeid in (1006, 1014)
      and p.amount > 0
      and properties::json->>'method' not in ('giftcard', 'paypal')
...
exports.isAmountRefundable = async function(client, orderid, totalamount) {
  const payments = await getMollieRefundablePayments(client, orderid)
