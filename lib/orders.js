...
exports.getTicketInfo = async function(client, orderids, inputOptions = {}) {
    ...
    for (const order of orderResult.data) {
      ...
      for (const ticket of order.tickets) {
        ...
        // Optionally skip historical or future events
        const future = (moment(orderResult.lookup.events[eventid].startts).diff(moment(), 'days') >= 0)
        const historical = (moment(orderResult.lookup.events[eventid].endts).diff(moment(), 'days') < 0)
        if (options.filterHistorical && historical) {
          continue;
        }
        if (options.filterFuture && future) {
          continue;
        }
