...
exports.getTicketInfo = async function(client, orderids, inputOptions = {}) {
    ...
    for (const orderid of orderids) {
      ...
      for (const ticket of order.tickets) {
        ...
        // Optionally skip historical events
        if (options.filterHistoral && moment(order.lookup.events[eventid].endts).diff(moment(), 'days') < 0) {
          continue;
        }
        ...
        ticket.pdfallowed = [15002,15003,15004].includes(order.lookup.deliveryscenarios[order.deliveryscenarioid].allowetickets)
...
async function getMolliePayments(client, orderid) {
    const sql = `
      select 
        ...
        coalesce(p.amount + sum(refunds.amount), 0) as refundable
      from tm.payment p
