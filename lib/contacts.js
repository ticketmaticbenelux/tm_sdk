...
exports.getConfig = async function(client) {
  ...
  const configs = await Promise.all(
    Object.entries(queries).map(async ([key, sql]) => ([key, await api.export(client, sql)]))
  )
...
exports.getPricetypeVoucherInfo = async function(client, contactid, filter = null) {
    ...
    const vouchers = await api.export(client, `
        ...
        where 
            ...
            ${sqlWhereVoucher}
        group by c.id, c.code, c.createdts, c.expiryts, v.namenl, o.id, v.validity
        order by c.id desc
    `)
