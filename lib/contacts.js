...
const { descend, fromPairs, sortWith, sum, pipe, pluck, prop, reject } = R
...
exports.getCustomFields = async function(client) {
  const l = client.language
  const sql = `
    SELECT DISTINCT ON (cf.id)
      cf.id,
      cf.key,
      cf.caption${l} AS caption,
      t.name${l} AS fieldtype,
      CASE
        WHEN requiredtypeid = 30003 then 'checkout'
        WHEN requiredtypeid = 30002 THEN 'everywhere'
        ELSE 'no'
      END AS required,      
      CASE 
          WHEN cf.fieldtypeid IN (12009, 12010, 12011, 12012)
            THEN json_agg(
              json_build_object('id', cfv.id, 'caption', cfv.caption${l}) 
              ORDER BY cfv.typeid) 
              FILTER (WHERE cfv.id IS NOT NULL
            )
          ELSE '[]'::json
      END AS customfieldvalues
    FROM 
      tm.customfield cf
    LEFT JOIN 
      tm.customfieldvalues cfv ON cf.id = cfv.typeid AND cfv.isarchived IS DISTINCT FROM TRUE
    LEFT JOIN
      tm.systemtype t ON t.id = cf.fieldtypeid
    WHERE 
      cf.typeid = 13002 
      AND cf.isarchived IS DISTINCT FROM TRUE
    GROUP BY 
      cf.id, cf.key, cf.requiredtypeid, cf.caption${l}, cf.fieldtypeid, t.name${l}
    ORDER BY 
      cf.id
  `
  const res = await api.export(client, sql)
  return res.map(row => ({
    ...row,
    customfieldvalues: JSON.parse(row.customfieldvalues)
  }));  
}

exports.getInfo = async function(client, contactid, customfieldkeys = []) {
  let selectFields = `customertitleid, firstname, middlename, lastname, DATE(birthdate) AS birthdate, languagecode`;

  if (customfieldkeys.length > 0) {
    const additionalFields = customfieldkeys.join(', ');
...
exports.getCreditInfo = async function(client, contactid, filter = null) {
    ...
    const vouchers = await api.export(client, `
        ...
        where 
            ...
            ${sqlWhereVoucher}
        order by c.expiryts asc, c.id desc
  `)
  ...
  const activeVouchers = reject(R.propEq('expired', true), vouchers)
  const totalamount = pipe(pluck("amount"), sum)(activeVouchers)
  const totalused = pipe(pluck("amountused"), sum)(activeVouchers)
  const meta = {
      totalamount,
      totalused,
...
exports.getPricetypeVoucherInfo = async function(client, contactid, filter = null) {
    ...
    const vouchers = await api.export(client, `
        ...
        from tm.vouchercode c
        inner join tm.orderproduct op on op.vouchercodeids @> to_jsonb(c.id)
        inner join tm.order o on o.id = op.orderid
        inner join tm.voucher v on v.id = c.voucherid
        ...
        where 
            ...
            ${sqlWhereVoucher}
        group by c.id, c.code, c.createdts, c.expiryts, v.name${l}, o.id, o.deliverystatus, v.validity
        order by c.id desc
    `)
