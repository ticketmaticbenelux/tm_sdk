...
const { fromPairs, sum, pipe, pluck } = R
...
exports.getInfo = async function(client, contactid) {
    const sql = `
      select 
        customertitleid, firstname, middlename, lastname, birthdate, languagecode
      from tm.contact
      where id = ${contactid}
    `
    const res = await api.export(client, sql)
...
async function getOrderidsForTickets(client, contactid, options) {
  let whereDate = ``
  if (options.filterHistorical) {
    whereDate = `and endts > now()`
  }
  if (options.filterFuture) {
    whereDate = `${whereDate} and startts < now()`
  }
...
exports.getTicketInfo = async function(client, contactid, inputOptions = {}) {
  const defaultOptions = {
    filterFuture: false,
    filterHistorical: true,
  }  
  const options = Object.assign({}, defaultOptions, inputOptions)
  const orderids = await getOrderidsForTickets(client, contactid, options)
  return orders.getTicketInfo(client, orderids, options)
}